...
import { MantenimientoProgramado, Empresa } from '@/types';
...
export default function MantenimientosProgramadosPage() {
  const [mantenimientos, setMantenimientos] = useState<MantenimientoProgramado[]>([]);
  const [filteredMantenimientos, setFilteredMantenimientos] = useState<MantenimientoProgramado[]>([]);
  ...
  const loadMantenimientos = async () => {
    try {
      ...
      const data = await api.getMantenimientosProgramados();
      ...
      toast.error('Error al cargar mantenimientos programados');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <Layout>
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Link
              href="/mantenimientos-programados/nuevo"
                        ...
                        <tr key={mant.id}>
                          ...
                          <td>
                            {mant.fechaProgramada
                              ? format(new Date(mant.fechaProgramada), 'dd/MM/yyyy')
                              : 'N/A'}
                          </td>
                          <td>{mant.tecnico?.nombreCompleto || 'Sin asignar'}</td>
                          <td>
                            <span
